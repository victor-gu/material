...
2. for in 循环判断
3. 使用ES6的Object.keys()方法将对象的key转化为数组，然后判断长度

#### Math.max找出数组中的最大项
> Math.max(param1,param2) 因为参数不支持数组。Math.max.apply(null,array)，这样就可以轻易的得到一个数组中最大的一项

#### Math.trunc()：es6中Math对象的拓展
> 该方法返回整数部分

* 对于非数值，Math.trunc会内部使用Number将其转为数值
* 对于空值和无法截取整数的值，返回NaN

#### Math.sign()：es6中Math对象的拓展
